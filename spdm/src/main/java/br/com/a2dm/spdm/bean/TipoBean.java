...
@RequestScoped
@ManagedBean
public class TipoBean extends AbstractBean<Tipo, TipoService>
{	
	private JSFUtil util = new JSFUtil();
	
	public TipoBean()
	{
		super(TipoService.getInstancia());
		this.ACTION_SEARCH = "tipo";
		this.pageTitle = "Manutenção / Tipo";
	...
	protected void validarInserir() throws Exception
	{
		if(this.getEntity() == null
				|| this.getEntity().getDesTipo() == null
				|| this.getEntity().getDesTipo().trim().equals(""))
		{
	...
	{		
		try
		{
			if(this.getEntity() != null)
			{
				...
				{
					TipoService.getInstancia().inativar(this.getEntity());
	...
	{		
		try
		{
			if(this.getEntity() != null)
			{
				...
				{
					TipoService.getInstancia().ativar(this.getEntity());
	...
	public void visualizar()
	{
		...
		{
			Tipo tipo = new Tipo();
			tipo.setIdTipo(this.getEntity().getIdTipo());
			
			tipo = TipoService.getInstancia().get(tipo, TipoService.JOIN_USUARIO_CAD
															   | TipoService.JOIN_USUARIO_ALT);
